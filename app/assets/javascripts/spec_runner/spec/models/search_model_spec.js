describe("SearchModel", function () {
	...
	describe("filters functionality", function () {
		...
		it("setCategoryGroup() sets category_group param as expected", function () {
			var searchModel = new SearchModel({
				selected_categories: ["a", "b", "c"],
				unselected_categories: ["d"]
			});

			searchModel.setCategoryGroup("aa");

			expect(searchModel.get("category_group")).toBe("aa");
			expect(searchModel.get("selected_categories").length).toBe(0);
			expect(searchModel.get("unselected_categories").length).toBe(0);
		});
	});
