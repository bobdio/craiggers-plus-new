...
var locationService = (function () {
	...
	*/
	...
	// copied from threetaps/views/search/standart_search_views/common_functions
	module.getLocationContext = function (code, level, cb) {
		...
		function locationFetched (loc, cb) {
			if (_.isString(loc)) {
				loc = {};
			}
			loc.has_context = true;
			var location = new LocationModel(loc);
					
			if (app.collections.locations) {
				app.collections.locations.add(location);
			}
		...
		function callback(position) {
			$.ajax({
				...
				success: function(data) {
					...
					module.getLocationContext(locationDeep.code, level, cb);
				},
