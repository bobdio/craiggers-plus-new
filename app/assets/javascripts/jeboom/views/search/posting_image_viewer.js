var PostingImageViewer = BaseView.extend({
    ...
    initialize: function() {
      ...
      this.$('.frame').html('')
      this.$('.frame').prepend($('<img>', {height: '300px'}).addClass('main').attr('src', posting.get('images')[0].full))
      .css('max-width', '400px')


      //set minis
      _.each(posting.get('images'), function(image) {
        viewer.$('.minis').append(
          $('<img>').addClass('mini').attr('src', image.full)
        );
        
        viewer.$('.images_for_fancybox').append(
          $('<a>', {class: 'fancybox_image', href: image.full, rel: 'gallery'})
        );
      });
