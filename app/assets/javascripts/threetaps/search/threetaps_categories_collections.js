Craiggers.Collections.ThreeTapsCategories = Backbone.Collection.extend({
  ...
  initialize: function() {
    ...
    $('#categories .code').each(function() {
      var el;
      var c = $(this);
      if ( c.parents('.subcat').length )
        el = c.parents('.subcat');
      else if ( c.parents('.cat').length )
        el =  c.parents('.cat');
  ...
  //getCategoryList is used for old autocomplete, which already does not work for location and category inputs on craiggers.
